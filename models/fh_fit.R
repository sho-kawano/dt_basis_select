...
fh_fit <- function(X, y, d.var, ndesired, nburn, nthin, var.prior = "improper",
  ...
  for (index in 1:(nsim + nburn)) {
    # update Beta
    Xstd <- X / sqrt(d.var)
    U <- chol(t(Xstd) %*% Xstd)  # Cholesky of precision matrix X' D^{-1} X
    a <- apply(X * (y - v) / d.var, 2, sum)
    b <- rnorm(j)
    beta <- backsolve(U, backsolve(U, a, transpose = TRUE) + b)
  ...
  return(list(
    beta = Res_beta, v = Res_v, sigma.sq = Res_sigma.sq, theta = Res_theta,
