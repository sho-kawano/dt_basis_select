...
#' converts `mcmc_mat` outputs to mean, median, variance, 90% credible interval
#' @param mcmc_mat mcmc results 
#' @param parameter parameter that you're interested in saving results for 
#' @param inv_transform inverse transform 
mcmc_summary <- function(mcmc_mat, parameter, method, 
                         inv_transform=function(x){x}, level=0.9){
  alpha = 1-0.9
  post.mean = mcmc_mat[[parameter]] %>% inv_transform() %>% colMeans()
  post.median = mcmc_mat[[parameter]] %>% inv_transform() %>% apply(2, median)
  post.var = mcmc_mat[[parameter]] %>% inv_transform() %>% apply(2, var)
  post.cr_int = mcmc_mat[[parameter]] %>% inv_transform() %>%  
    apply(2, function(x){quantile(x, c(alpha/2, 1-alpha/2))}) %>% t() 
  colnames(post.cr_int) = c("lower", "upper")
  
  return(cbind(mean=post.mean, 
                    median=post.median, 
                    var=post.var,
                    cr_int = post.cr_int
  ) %>% as.data.frame() %>% 
