...
make_plugin_nll <- function(z_test_rep, eps_rep, chosen_var, n_folds) {
  function(theta.hat, j) {
    if (n_folds == 1) {
      ...
      sd_pred <- sqrt((1 - eps_rep) * chosen_var)
    } else {
      # For k>1: z_train~N((1-eps)*theta,...), z_test~N(eps*theta,...)
      # theta.hat estimates (1-eps)*theta
      mean_pred <- (eps_rep / (1 - eps_rep)) * theta.hat
      sd_pred <- sqrt(eps_rep * chosen_var)
    }
    -1 * sum(dnorm(z_test_rep[, j], mean_pred, sd_pred, log = TRUE))
  }
}

make_predictive_nll <- function(z_test_rep, eps_rep, chosen_var, n_folds) {
  function(theta.hat, theta.var, j) {
    if (n_folds == 1) {
      # For k=1
      mean_pred <- ((1 - eps_rep) / eps_rep) * theta.hat
      sd_pred <- sqrt((1 - eps_rep) * chosen_var + (1 - eps_rep)^2 * theta.var)
    } else {
      # For k>1
      mean_pred <- (eps_rep / (1 - eps_rep)) * theta.hat
      sd_pred <- sqrt(eps_rep * chosen_var + (eps_rep / (1 - eps_rep))^2 * theta.var)
    }
    -1 * sum(dnorm(z_test_rep[, j], mean_pred, sd_pred, log = TRUE))
  }
}

make_mse <- function(z_test_rep, eps_rep, chosen_var, n_folds) {
  function(theta.hat, j) {
    if (n_folds == 1) {
      ...
      bias_correction <- mean(chosen_var / (1 - eps_rep))
    } else {
      ...
      bias_correction <- mean(chosen_var / eps_rep)
    }
...
make_naive_mse <- function(z_test_rep, eps_rep, chosen_var, n_folds) {
  function(theta.hat, j) {
    if (n_folds == 1) {
      de_thinned_hat <- theta.hat / eps_rep
      de_thinned_test <- z_test_rep[, j] / (1 - eps_rep)
...
summary_dt <- function(comp_no, n_folds, chosen_var, loss_function, all_data, results_dir, n_reps_to_use, eps = NULL) {
  #-----------------------------------------------------------------------------
  ...
  result_all_reps <- foreach(rep = 1:n_reps_to_use, .combine = rbind) %do% {
    ...
    result_one_rep <- foreach(j = 1:n_folds, .combine = rbind) %do% {
      ...
      loss_fn <- switch(loss_function,
        "plugin_NLL" = make_plugin_nll(z_test_rep, eps_scalar, chosen_var, n_folds),
        "predictive_NLL" = make_predictive_nll(z_test_rep, eps_scalar, chosen_var, n_folds),
        "MSE" = make_mse(z_test_rep, eps_scalar, chosen_var, n_folds),
        "naive_MSE" = make_naive_mse(z_test_rep, eps_scalar, chosen_var, n_folds),
