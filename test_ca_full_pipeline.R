#!/usr/bin/env Rscript
# ==============================================================================
# CA Full Pipeline Test - 10 Comparisons
# ==============================================================================
# Purpose: Test complete pipeline (DT + ESIM) before production run
# Based on: CA employed, equal_75 (winner from oracle analysis)
# ==============================================================================
...
cat("\n")
cat("================================================================================\n")
cat("  CA EMPLOYED: FULL PIPELINE TEST (10 COMPARISONS)\n")
cat("================================================================================\n\n")
...
model_config <- list(
  ...
  eps_values = c(0.3, 0.5), # Just 2 epsilon values for testing
  n_reps_dt = c(1, 3), # 1 and 3 repetitions
  k_folds = 5, # Multi-fold DT

  # Empirical simulation settings
  n_iter_esim = 50, # Reduced for testing (use 100 for production)

  # Computational settings
  n_cores = 10,

  # Output
  results_dir = "_results_ca_test_pipeline",

  # Study design
  n_comp = 10 # Number of comparisons
)
...
for (eps in model_config$eps_values) {
  for (n_rep in model_config$n_reps_dt) {
    cat(sprintf("  eps=%.2f, n_reps=%d\n", eps, n_rep))

    foreach(comp_no = 1:model_config$n_comp) %dopar% {
      run_dt(comp_no,
        k = 1, results_dir = model_config$results_dir,
        model_config = model_config, eps = eps, n_reps = n_rep
      )
    }
  }
}
...
for (comp_no in 1:model_config$n_comp) {
  cat(sprintf("  Comparison %d/%d\n", comp_no, model_config$n_comp))
  run_esim(comp_no, model_config$n_cores, model_config$results_dir,
    model_config,
...
cat(sprintf("✓ Configuration saved to %s\n\n", file.path(model_config$results_dir, "model_config.RDS")))
...
cat("\n")
cat("================================================================================\n")
cat("  CA EMPLOYED FULL PIPELINE TEST COMPLETE\n")
cat("================================================================================\n")
cat(sprintf("Results saved to: %s\n\n", model_config$results_dir))
cat("Next steps:\n")
cat("  Run summary script: Rscript run_summary.R %s\n\n", model_config$results_dir)
